...
// Middleware globals — request counter for the middleware demo
...
pub fn main(init: std.process.Init) !void {
    ...
    try server.use(requestInfoMiddleware);
...
// Middleware: attaches a request ID and timestamp to every request via http.assign().
// Downstream handlers read the data with http.assigned().
...
fn middlewareDemo(http: *HTTPRequest) !void {
    ...
    try sse.patchElementsFmt(
        ...
        \\  <p class="text-sm opacity-70">Middleware attached this data via <code>http.assign()</code></p>
